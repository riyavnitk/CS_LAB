...
s=[0 sqrt(6)]; %Non-Centrality Parameter
plotStyle={'b-','r-'}; %Simulating the PDF from two Gaussian Random Variables
t = 0:0.1:100;
for i = 1:length(s)
X = s(i) + sigma.*sin(t); %Gaussian RV with mean=s and given sigma
Y = 0 + sigma.*sin(t); %Gaussian RV with mean=0 and same sigma as Y
Z=X+1i*Y;
